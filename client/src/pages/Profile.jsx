...
export default function Profile() {
  ...
  const [userListings, setUserListings] = useState([]);  const dispatch = useDispatch();
  useEffect(() => {
    if (file) {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.text(); // Get the response text instead of parsing as JSON
    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      {userListings.map((listing) => (
        <div key={listing._id} className="border rounded-lg p-3 flex justify-between item-center gap-4k">
          ...
          <Link className="text-slate-700 font-semibold flex-1 hover:underline trunket" to={`/listing/${listing._id}`}>
            <p >{listing.name}</p>
